...
static int team69_kyber_domain_wake(wait_queue_entry_t *wait, unsigned mode, int flags,
			     void *key);

static unsigned int team69_kyber_sched_domain(unsigned int op)
{
...
static void team69_kyber_resize_domain(struct kyber_queue_data *kqd,
...
static void team69_kyber_timer_fn(struct timer_list *t)
{
	...
	for (sched_domain = 0; sched_domain < KYBER_OTHER; sched_domain++) {
		...
		if (bad || p99 >= KYBER_GOOD_BUCKETS) {
			...
			team69_kyber_resize_domain(kqd, sched_domain, depth);
		}
	}
}

static unsigned int team69_kyber_sched_tags_shift(struct request_queue *q)
{
	/*
...
static struct kyber_queue_data *team69_kyber_queue_data_alloc(struct request_queue *q)
{
	...
	timer_setup(&kqd->timer, team69_kyber_timer_fn, 0);
	...
	shift = team69_kyber_sched_tags_shift(q);
...
static int team69_kyber_init_sched(struct request_queue *q, struct elevator_type *e)
{
	struct kyber_queue_data *kqd;
	...
	kqd = team69_kyber_queue_data_alloc(q);
	if (IS_ERR(kqd)) {
		kobject_put(&eq->kobj);
		return PTR_ERR(kqd);
	}
...
static void team69_kyber_exit_sched(struct elevator_queue *e)
{
...
static void team69_kyber_ctx_queue_init(struct kyber_ctx_queue *kcq)
{
	unsigned int i;
...
static int team69_kyber_init_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
	...
	for (i = 0; i < hctx->nr_ctx; i++)
		team69_kyber_ctx_queue_init(&khd->kcqs[i]);
...
static void team69_kyber_exit_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
				  ...
				  struct request *rq)
{
	...
	if (nr != -1) {
		sched_domain = team69_kyber_sched_domain(rq->cmd_flags);
		sbitmap_queue_clear(&kqd->domain_tokens[sched_domain], nr,
				    rq->mq_ctx->cpu);
	}
}

static void team69_kyber_limit_depth(unsigned int op, struct blk_mq_alloc_data *data)
{
	/*
...
static bool team69_kyber_bio_merge(struct blk_mq_hw_ctx *hctx, struct bio *bio,
		unsigned int nr_segs)
{
	struct kyber_hctx_data *khd = hctx->sched_data;
	struct blk_mq_ctx *ctx = blk_mq_get_ctx(hctx->queue);
	struct kyber_ctx_queue *kcq = &khd->kcqs[ctx->index_hw[hctx->type]];
	unsigned int sched_domain = team69_kyber_sched_domain(bio->bi_opf);
...
static void team69_kyber_prepare_request(struct request *rq, struct bio *bio)
{
	rq_set_domain_token(rq, -1);
}

static void team69_kyber_insert_requests(struct blk_mq_hw_ctx *hctx,
				  struct list_head *rq_list, bool at_head)
{
	...
	list_for_each_entry_safe(rq, next, rq_list, queuelist) {
		unsigned int sched_domain = team69_kyber_sched_domain(rq->cmd_flags);
...
static void team69_kyber_finish_request(struct request *rq)
{
...
static void team69_kyber_completed_request(struct request *rq, u64 now)
{
	struct kyber_queue_data *kqd = rq->q->elevator->elevator_data;
	struct kyber_cpu_latency *cpu_latency;
	unsigned int sched_domain;
...
static void team69_kyber_flush_busy_kcqs(struct kyber_hctx_data *khd,
				  unsigned int sched_domain,
				  struct list_head *list)
{
...
static int team69_kyber_domain_wake(wait_queue_entry_t *wqe, unsigned mode, int flags,
			     void *key)
{
			  ...
			  struct blk_mq_hw_ctx *hctx)
{
	...
	if (rq) {
		...
		nr = kyber_get_domain_token(kqd, khd, hctx);
		if (nr >= 0) {
			team69_kyber_flush_busy_kcqs(khd, khd->cur_domain, rqs);
...
static struct request *team69_kyber_dispatch_request(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_queue_data *kqd = hctx->queue->elevator->elevator_data;
	struct kyber_hctx_data *khd = hctx->sched_data;
	struct request *rq;
	...
	spin_lock(&khd->lock);
	printk("In team69_kyber_dispatch_request function\n");
	/*
...
static bool team69_kyber_has_work(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_hctx_data *khd = hctx->sched_data;
	int i;
...
static ssize_t team69_kyber_##name##_lat_show(struct elevator_queue *e,	\
				       char *page)			\
{									\
	struct kyber_queue_data *kqd = e->elevator_data;		\
									\
	return sprintf(page, "%llu\n", kqd->latency_targets[domain]);	\
									...
									\
static ssize_t team69_kyber_##name##_lat_store(struct elevator_queue *e,	\
...
#define KYBER_LAT_ATTR(op) __ATTR(op##_lat_nsec, 0644, team69_kyber_##op##_lat_show, team69_kyber_##op##_lat_store)
...
static int team69_kyber_##name##_tokens_show(void *data, struct seq_file *m)	\
{									\
	...
	return 0;							\
									...
									\
static void *team69_kyber_##name##_rqs_start(struct seq_file *m, loff_t *pos)	\
	__acquires(&khd->lock)						\
{									\
	...
	return seq_list_start(&khd->rqs[domain], *pos);			\
									...
									\
static void *team69_kyber_##name##_rqs_next(struct seq_file *m, void *v,	\
				     loff_t *pos)			\
{									\
	struct blk_mq_hw_ctx *hctx = m->private;			\
	struct kyber_hctx_data *khd = hctx->sched_data;			\
	...
	return seq_list_next(v, &khd->rqs[domain], pos);		\
									...
									\
static void team69_kyber_##name##_rqs_stop(struct seq_file *m, void *v)	\
	__releases(&khd->lock)						\
{									\
	struct blk_mq_hw_ctx *hctx = m->private;			\
	struct kyber_hctx_data *khd = hctx->sched_data;			\
...
static const struct seq_operations kyber_##name##_rqs_seq_ops = {	\
	.start	= team69_kyber_##name##_rqs_start,				\
	.next	= team69_kyber_##name##_rqs_next,				\
	.stop	= team69_kyber_##name##_rqs_stop,				\
	.show	= blk_mq_debugfs_rq_show,				\
};									\
									\
static int team69_kyber_##name##_waiting_show(void *data, struct seq_file *m)	\
{									\
...
#define KYBER_QUEUE_DOMAIN_ATTRS(name)	\
	{#name "_tokens", 0400, team69_kyber_##name##_tokens_show}
...
#define KYBER_HCTX_DOMAIN_ATTRS(name)					\
	...
	{#name "_waiting", 0400, team69_kyber_##name##_waiting_show}
...
static struct elevator_type team69_kyber_sched = {
	.ops = {
		.init_sched = team69_kyber_init_sched,
		.exit_sched = team69_kyber_exit_sched,
		.init_hctx = team69_kyber_init_hctx,
		.exit_hctx = team69_kyber_exit_hctx,
		.limit_depth = team69_kyber_limit_depth,
		.bio_merge = team69_kyber_bio_merge,
		.prepare_request = team69_kyber_prepare_request,
		.insert_requests = team69_kyber_insert_requests,
		.finish_request = team69_kyber_finish_request,
		.requeue_request = team69_kyber_finish_request,
		.completed_request = team69_kyber_completed_request,
		.dispatch_request = team69_kyber_dispatch_request,
		.has_work = team69_kyber_has_work,
	},
#ifdef CONFIG_BLK_DEBUG_FS
	...
	.elevator_name = "team69_kyber",
	.elevator_owner = THIS_MODULE,
};

static int __init team69_kyber_init(void)
{
	return elv_register(&team69_kyber_sched);
}

static void __exit team69_kyber_exit(void)
{
	elv_unregister(&team69_kyber_sched);
}

module_init(team69_kyber_init);
module_exit(team69_kyber_exit);

MODULE_AUTHOR("team69");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("team69_Kyber I/O scheduler");
