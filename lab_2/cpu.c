...
int main(int argc, char *argv[])
{
	...
	fprintf(fptr,"max total waiting time: %lf !\n", max_wt);
	fclose(fptr);
...
//				printf("CDA: thread %s remaining %lld, clocktick %llu\n", current->thread_info->processName, current->thread_info->work_duration, clocktick);
			...
			if(current->thread_info->work_duration == 0) {
				//fprintf(fptr,"Total wait time for INT/VE proccess %s = %lf\n",current->thread_info->processName, current->total_wait_time);
...
static void killtask(struct task_struct **p)
{
	...
	double turn_around_time = sched_clock() - current->spawned_time;

	printf("###-Process: %s is going down-###\n", j->thread_info->processName); 
	fprintf(fptr,"Total wait time for proccess %s = %lf\n",current->thread_info->processName, current->total_wait_time);
	fprintf(fptr, "(%lld): Turn Around Time (TAT) for process %s: %lf\n", sched_clock(), current->thread_info->processName, turn_around_time);
	
	if(current->total_wait_time > max_wt){
		max_wt = current->total_wait_time;
		//max_wt_ptr->thread_info->processName = current->thread_info->processName;
	}
